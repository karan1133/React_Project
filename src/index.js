import React from "react";
import ReactDom from "react-dom";
import Home from "./component/Home";
import Professionals from "./component/Professionals";
import { BrowserRouter,Routes,Route } from "react-router-dom";
import Pfhome from "./component/Pfhome";
import Products from "./component/Products";
import Products2 from "./component/Products2";
import Products3 from "./component/Products3";
import Products4 from "./component/Products4";
import Living from "./component/Living";
import About from "./component/About";
import Magazine from "./component/Magazine";
import Card1 from "./component/Card1";
import Card2 from "./component/Card2";
import Card3 from "./component/Card3";
import Card4 from "./component/Card4";
import Card5 from "./component/Card5";
import Card6 from "./component/Card6";
import Card7 from "./component/Card7";
import Card8 from "./component/Card8";
import Card9 from "./component/Card9";
import Card10 from "./component/Card10";
import Card11 from "./component/Card11";
import Card12 from "./component/Card12";
import Card13 from "./component/Card13";
import Card14 from "./component/Card14";
import Card15 from "./component/Card15";
import Card16 from "./component/Card16";
import Card17 from "./component/Card17";
import Card18 from "./component/Card18";
import Card19 from "./component/Card19";
import Card20 from "./component/Card20";
import Card21 from "./component/Card21";
import Card22 from "./component/Card22";
import Card23 from "./component/Card23";
import Card24 from "./component/Card24";
import Card25 from "./component/Card25";
import Card26 from "./component/Card26";
import Card27 from "./component/Card27";
import Card28 from "./component/Card28";
import Card29 from "./component/Card29";
import Card30 from "./component/Card30";
import Card31 from "./component/Card31";
import Card32 from "./component/Card32";
import Card33 from "./component/Card33";
import Card34 from "./component/Card34";
import Card35 from "./component/Card35";
import Card36 from "./component/Card36";
import Card37 from "./component/Card37";
import Card38 from "./component/Card38";
import Card39 from "./component/Card39";
import Card40 from "./component/Card40";
import Card41 from "./component/Card41";
import Card42 from "./component/Card42";
import Card43 from "./component/Card43";
import Card44 from "./component/Card44";
import Card45 from "./component/Card45";
import Card46 from "./component/Card46";
import Card47 from "./component/Card47";
import Card48 from "./component/Card48";
import Card49 from "./component/Card49";
import Card50 from "./component/Card50";
import Card51 from "./component/Card51";
import Card52 from "./component/Card52";
import Card53 from "./component/Card53";
import Card54 from "./component/Card54";
import Card55 from "./component/Card55";
import Card56 from "./component/Card56";
import Card57 from "./component/Card57";
import Card58 from "./component/Card58";
import Card59 from "./component/Card59";
import Card60 from "./component/Card60";
import Card61 from "./component/Card61";
import Card62 from "./component/Card62";
import Card63 from "./component/Card63";
import Card64 from "./component/Card64";
import Card65 from "./component/Card65";
import Card66 from "./component/Card66";
import Login from "./component/Login";

ReactDom.render(
  <>
    <BrowserRouter>
      <Routes>
        <Route path="/" element={<Home/>}></Route>
        <Route path="/home" element={<Home/>}></Route>
        <Route path="/professionals" element={<Professionals/>}></Route>
        <Route path="/pfhome" element={<Pfhome/>}></Route>
        <Route path="/products" element={<Products/>}></Route>
        <Route path="/products2" element={<Products2/>}></Route>
        <Route path="/products3" element={<Products3/>}></Route>
        <Route path="/products4" element={<Products4/>}></Route>
        <Route path="/living" element={<Living/>}></Route>
        <Route path="/about" element={<About/>}></Route>
        <Route path="/magazine" element={<Magazine/>}></Route>
        <Route path="/card1" element={<Card1/>}></Route>
        <Route path="/card2" element={<Card2/>}></Route>
        <Route path="/card3" element={<Card3/>}></Route>
        <Route path="/card4" element={<Card4/>}></Route>
        <Route path="/card5" element={<Card5/>}></Route>
        <Route path="/card6" element={<Card6/>}></Route>
        <Route path="/card7" element={<Card7/>}></Route>
        <Route path="/card8" element={<Card8/>}></Route>
        <Route path="/card9" element={<Card9/>}></Route>
        <Route path="/card10" element={<Card10/>}></Route>
        <Route path="/card11" element={<Card11/>}></Route>
        <Route path="/card12" element={<Card12/>}></Route>
        <Route path="/card13" element={<Card13/>}></Route>
        <Route path="/card14" element={<Card14/>}></Route>
        <Route path="/card15" element={<Card15/>}></Route>
        <Route path="/card16" element={<Card16/>}></Route>
        <Route path="/card17" element={<Card17/>}></Route>
        <Route path="/card18" element={<Card18/>}></Route>
        <Route path="/card19" element={<Card19/>}></Route>
        <Route path="/card20" element={<Card20/>}></Route>
        <Route path="/card21" element={<Card21/>}></Route>
        <Route path="/card21" element={<Card21/>}></Route>
        <Route path="/card23" element={<Card23/>}></Route>
        <Route path="/card24" element={<Card24/>}></Route>
        <Route path="/card25" element={<Card25/>}></Route>
        <Route path="/card26" element={<Card26/>}></Route>
        <Route path="/card27" element={<Card27/>}></Route>
        <Route path="/card28" element={<Card28/>}></Route>
        <Route path="/card29" element={<Card29/>}></Route>
        <Route path="/card30" element={<Card30/>}></Route>
        <Route path="/card31" element={<Card31/>}></Route>
        <Route path="/card32" element={<Card32/>}></Route>
        <Route path="/card33" element={<Card33/>}></Route>
        <Route path="/card34" element={<Card34/>}></Route>
        <Route path="/card35" element={<Card35/>}></Route>
        <Route path="/card36" element={<Card36/>}></Route>
        <Route path="/card37" element={<Card37/>}></Route>
        <Route path="/card38" element={<Card38/>}></Route>
        <Route path="/card39" element={<Card39/>}></Route>
        <Route path="/card40" element={<Card40/>}></Route>
        <Route path="/card41" element={<Card41/>}></Route>
        <Route path="/card42" element={<Card42/>}></Route>
        <Route path="/card43" element={<Card43/>}></Route>
        <Route path="/card44" element={<Card44/>}></Route>
        <Route path="/card45" element={<Card45/>}></Route>
        <Route path="/card46" element={<Card46/>}></Route>
        <Route path="/card47" element={<Card47/>}></Route>
        <Route path="/card48" element={<Card48/>}></Route>
        <Route path="/card49" element={<Card49/>}></Route>
        <Route path="/card50" element={<Card50/>}></Route>
        <Route path="/card51" element={<Card51/>}></Route>
        <Route path="/card52" element={<Card52/>}></Route>
        <Route path="/card53" element={<Card53/>}></Route>
        <Route path="/card54" element={<Card54/>}></Route>
        <Route path="/card55" element={<Card55/>}></Route>
        <Route path="/card56" element={<Card56/>}></Route>
        <Route path="/card57" element={<Card57/>}></Route>
        <Route path="/card58" element={<Card58/>}></Route>
        <Route path="/card59" element={<Card59/>}></Route>
        <Route path="/card60" element={<Card60/>}></Route>
        <Route path="/card61" element={<Card61/>}></Route>
        <Route path="/card62" element={<Card62/>}></Route>
        <Route path="/card63" element={<Card63/>}></Route>
        <Route path="/card64" element={<Card64/>}></Route>
        <Route path="/card65" element={<Card65/>}></Route>
        <Route path="/card66" element={<Card66/>}></Route>
        <Route path="/login" element={<Login/>}></Route>
      </Routes>
    </BrowserRouter>
  </>,
  document.getElementById('root')
)
  
  
